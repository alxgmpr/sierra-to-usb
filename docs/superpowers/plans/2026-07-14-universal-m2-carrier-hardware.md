...
- [ ] **Step 3: Wire the sheet** per this table (CH224K per its datasheet application circuit as already redone by the user; TPS23730 per TI datasheet typical application at Class 4 with the Task-1 transformer; resistor values from Task 2 step 3):
...
| PoE front | `POE_VA±`,`POE_VB±` → BR1/BR2 → `POE_VDD54`/`POE_RTN`; D7 SMAJ58A across; 25.5 kΩ detection + Class-4 resistors per TPS23730 datasheet |
...
- [ ] **Step 2:** Run — FAIL.
- [ ] **Step 3: Wire:** J1 SSTX1/SSRX1/SSTX2/SSRX2 → D1/D2 (flow-through) → U1 connector-side per HD3SS3220 datasheet mapping (Task 2 step 3). U1 module side: TX pair → 220 nF ×2 → `SS_MOD_TX_C_P/N` (→CN1 35/37, via D3 flow-through); RX pair ← `SS_MOD_RX_P/N` (CN1 29/31, via D3, **no caps**). U1 straps: PORT=GND, ENn_MUX=GND, ADDR=NC, VBUS_DET←`VBUS_DATA` via 900 kΩ (`HD_VBUSDET`), VDD5←`VBUS_DATA`, VCC33←`+3V3_MUX` from U4 (input `VBUS_DATA`). J1 D± both rows → D4 → `USB2_CON_DP/DM` hierarchical labels; CC1/CC2 → U1 CC pins through D4 channels. J1 VBUS → `VBUS_DATA`; SMAJ5.0A across it (add refdes D8 — update refdes map comment inline).
